...
function persistApi() {
  return require('./github-persist');
}

// Локальные бэкапы (SQLite-снапшоты + фото на диске) имеет смысл создавать
// только в продакшене: там локальный диск эфемерный, и снапшоты уходят на
// GitHub. В локальной тестовой версии (PKB_ENV=dev / data/proverki-dev.db)
// бэкапы только засоряют диск разработчика, поэтому создание полностью
// отключаем.
...
function snapshotPhotosPath(backupId) {
  return path.join(backupsRoot(), 'snapshots', backupId, 'photos');
}
...
async function createBackupFromLive(atDate) {
  if (!backupsEnabled()) {
    return { skipped: true, reason: 'local_dev_disabled' };
  }
  ...
  const livePhotos = path.join(path.dirname(getDbPath()), 'photos');
  const backupPhotos = snapshotPhotosPath(id);
  let photoCount = 0;
  if (fs.existsSync(livePhotos)) {
    fs.cpSync(livePhotos, backupPhotos, { recursive: true });
    fs.readdirSync(backupPhotos).forEach(function(name) {
      if (!/\.avif$/i.test(name)) fs.rmSync(path.join(backupPhotos, name), { force: true });
    });
    photoCount = fs.readdirSync(backupPhotos).filter(function(name) {
      return fs.statSync(path.join(backupPhotos, name)).isFile();
    }).length;
  }
  const entry = { id, label, createdAt: when.getTime(), files: ['proverki.db', 'photos/'], photoCount, format: 'sqlite+photos' };
  ...
  persistApi().pushBackupToGithub(id, manifest).catch(function(e) {
    console.warn('[backups] GitHub sync failed:', e.message);
  });
   return { id, label, files: photoCount + 1, photoCount, format: 'sqlite+photos' };
}
...
async function restoreBackup(backupId) {
  ...
  const livePhotos = path.join(path.dirname(getDbPath()), 'photos');
  const backupPhotos = snapshotPhotosPath(backupId);
  fs.rmSync(livePhotos, { recursive: true, force: true });
  if (fs.existsSync(backupPhotos)) fs.cpSync(backupPhotos, livePhotos, { recursive: true });
  getDb({ reopen: true });

  manifest.activeBackupId = backupId;
  writeManifest(manifest);
  persistApi().scheduleDbPersist();
  persistApi().pushBackupToGithub(backupId, manifest).catch(function(e) {
    console.warn('[backups] GitHub sync after restore failed:', e.message);
  });
...
async function getBackupsState() {
  ...
  const { pullBackupsFromGithub } = require('./github-persist');
  try {
    await pullBackupsFromGithub();
  } catch (e) {
    console.warn('[backups] GitHub pull failed:', e.message);
  }
