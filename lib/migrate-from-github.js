...
const { pullDbFromGithub, isEnabled: isGithubPersistEnabled } = require('./github-persist');
const {
  clearAllRecords,
  importRecordRow,
  renumberYear,
...
const SQLITE_REMOTE_PATH = 'database/proverki.db';
...
async function migrateFromGithubIfEmpty() {
  ...
  if (isGithubPersistEnabled()) {
    const meta = await ghGetMeta(SQLITE_REMOTE_PATH);
    if (meta && meta.sha && (meta.size || 0) > 10000) {
      console.log('[migrate] GitHub SQLite found — retry pull instead of legacy Excel import');
      const pull = await pullDbFromGithub();
      closeDb();
      getDb({ reopen: true });
      const total = countAllRecords();
      if (total > 0) {
        console.log('[migrate] loaded', total, 'records from GitHub SQLite');
        return { total: total, dbPath: resolveDbPath(), source: 'sqlite' };
      }
      console.warn('[migrate] GitHub SQLite pull did not yield records:', pull.reason || 'unknown');
      return null;
    }
  }

  console.log('[migrate] empty database — importing legacy Excel/JSON from GitHub...');
