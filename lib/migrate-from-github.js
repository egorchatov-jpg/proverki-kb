...
async function ghListExcel(owner, repo) {
  const t = githubToken();
  const r = await fetch(ghUrl(owner, repo, ''), {
    headers: {
      Authorization: `token ${t}`,
...
async function importFromGithub(options) {
  ...
  const settingsRaw = await ghGetRaw(owner, repo, 'settings.json');
  if (settingsRaw) importSettings(JSON.parse(settingsRaw));

  const clRaw = await ghGetRaw(owner, repo, 'checklists.json');
  if (clRaw) {
    const cl = JSON.parse(clRaw);
    importChecklistItems(cl.items || {});
  }

  const subsRaw = await ghGetRaw(owner, repo, 'subscriptions.json');
  if (subsRaw) {
  ...
  for (const name of files) {
    ...
    const buf = await ghGetRaw(owner, repo, name);
    if (!buf) continue;
