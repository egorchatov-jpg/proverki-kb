...
const { closeDb, resolveDbPath, getDbPath, getDb } = require('./db');
const {
  githubToken,
  ghGetMeta,
  ghGetRaw,
...
async function pushDbToGithub() {
  ...
  return enqueuePush(async function() {
    ...
    if (localCount <= 0) {
      console.warn('[persist] refusing to push empty database to GitHub');
    ...
    if (remoteSize > 100000 && localCount < 10) {
      console.warn('[persist] refusing to overwrite large remote database with nearly empty local copy');
...
async function pullDbFromGithub() {
  ...
  if (fs.existsSync(localPath)) {
    ...
    if (!localEmpty && !localCorrupt && remoteTime > 0 && localStat.size > 0 && localStat.mtimeMs >= remoteTime - 2000) {
      return { pulled: false, reason: 'local_newer_or_same' };
    }
    if (!localEmpty && !localCorrupt && localStat.size > 0 && remoteSize > 0 && localStat.size >= remoteSize * 0.85) {
  ...
  return { pulled: true, bytes: buf.length, records: pulledCount };
}
