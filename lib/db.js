...
CREATE TABLE IF NOT EXISTS photos (
  ...
  violation_index INTEGER NOT NULL DEFAULT 0,
  uploaded_at   INTEGER NOT NULL DEFAULT (unixepoch())
);
CREATE INDEX IF NOT EXISTS idx_photos_check_id ON photos(check_id);
`;

function initSchema(db) {
  db.exec(SCHEMA_SQL);
  try { db.exec('ALTER TABLE photos ADD COLUMN client_photo_id TEXT'); } catch (_e) {}
  try { db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_photos_client_id ON photos(client_photo_id) WHERE client_photo_id IS NOT NULL'); } catch (_e) {}
  db.prepare("INSERT OR IGNORE INTO schema_meta (key, value) VALUES ('db_version', '1')").run();
  migrateDropInspectorColumn(db);
}

// pkb-v449: "Проверяющая организация" (колонка inspector) убрана из UI и
// Excel-экспорта — поле "Проверяющий" (бывший "Метод проверки") теперь
// содержит название проверяющей организации. Физически удаляем колонку из
// SQLite (ALTER TABLE ... DROP COLUMN поддерживается node:sqlite/SQLite
// 3.35+). Также переносим устаревшие значения "Самооценка"/"Самопроверка" в
// "Техресурс" — прямо в SQLite, а не только на клиенте: клиентская миграция
// migrateSamoproverkaToSamootsenka() правит только appState.db в браузере и
// никогда не отправляет изменение на сервер, поэтому старое значение из
// SQLite/GitHub возвращалось при каждой следующей загрузке (pkb-v457).
function migrateDropInspectorColumn(db) {
  const hasInspector = db.prepare("SELECT 1 FROM pragma_table_info('records') WHERE name = 'inspector'").get();
  if (hasInspector) {
    try {
      db.exec('ALTER TABLE records DROP COLUMN inspector');
    } catch (e) {
      console.warn('[db] failed to drop inspector column:', e.message);
    }
  }
  try {
    db.prepare("UPDATE records SET method = 'Техресурс' WHERE method IN ('Самооценка', 'Самопроверка')").run();
  } catch (e) {
    console.warn('[db] failed to migrate Самооценка/Самопроверка -> Техресурс:', e.message);
  }
}
