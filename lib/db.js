...
// 3.35+). Также переносим устаревшее значение "Самооценка" в "Техресурс".
function migrateDropInspectorColumn(db) {
  ...
  try {
    db.prepare("UPDATE records SET method = 'Техресурс' WHERE method = 'Самооценка'").run();
  } catch (e) {
    console.warn('[db] failed to migrate Самооценка -> Техресурс:', e.message);
  }
}
