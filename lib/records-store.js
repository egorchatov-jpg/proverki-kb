...
function selectAllRecords(db) {
  return db.prepare('SELECT * FROM records').all().map(rowToRecord);
}
...
function listRecordsByYear(year) {
  ...
  const records = getDb().prepare('SELECT * FROM records WHERE year = ?').all(y).map(rowToRecord);
  records.sort(cmpRecords);
  records.forEach(function(r, i) { r.num = i + 1; });
  return records;
}
...
function nextCheckIdForYear(year) {
  ...
  const rows = getDb().prepare('SELECT check_id FROM records WHERE year = ?').all(parseInt(String(year), 10));
  let maxSeq = 0;
...
function insertRecord(record, barriersConfig) {
  ...
  const existing = selectAllRecords(db);
  if (existing.some(r => recordFingerprint(r) === fp)) {
  ...
  db.prepare(`
    INSERT INTO records (
      check_id, num, year, date_check, date_entry, method, inspector, org, obj, curator,
  ...
  const saved = db.prepare('SELECT num FROM records WHERE check_id = ?').get(record.checkId);
  record.num = saved ? saved.num : 0;
  record.year = String(year);
  return { duplicate: false, num: record.num, year: String(year), checkId: record.checkId };
}
...
function updateRecord(dateEntry, fields, fallback) {
  const db = getDb();
  ...
  renumberYear(db, year);
  return findRecord(dateEntry, fallback);
}
...
function deleteRecordByKey(record) {
  ...
  renumberYear(db, year);
  return { success: true, year: String(year), deletedKey, file: null, checklists: { removed: 1 } };
}
