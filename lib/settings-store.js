...
const EMPTY_SETTINGS = {
  ...
  passwords: { admin: null, inspector: null, superuser: null, orgs: {}, hashes: { orgs: {} }, versions: { admin: 1, inspector: 1, superuser: 1, orgs: {} } },
  usedPasswords: [],
};
...
function savePasswordHashes(hashes, versions) {
  const db = getDb();
  const current = loadSettings();
  current.passwords = current.passwords || { admin: null, inspector: null, orgs: {} };
  current.passwords.hashes = hashes;
  if (versions) current.passwords.versions = versions;
  current.settingsUpdatedAt = Date.now();
  db.prepare(`INSERT INTO app_settings (id, json, updated_at) VALUES (1, ?, unixepoch()) ON CONFLICT(id) DO UPDATE SET json = excluded.json, updated_at = unixepoch()`)
    .run(JSON.stringify(current));
  return current;
}

function passwordVersionFor(role, orgName, passwords) {
  const versions = (passwords && passwords.versions) || {};
  if (role === 'org') return Number((versions.orgs || {})[orgName]) || 1;
  return Number(versions[role]) || 1;
}
...
/**
 * Remove whole barrier-years from all per-year settings maps (config,
 * custom passports, PK) — used for explicit admin year deletion. The regular
 * merge (mergeSettingsPreserveAll) deliberately re-adds server years that a
 * stale/partial client payload omits (multi-user protection); an explicit
 * deletion must be applied to the baseline BEFORE merge so it is not
 * resurrected, and re-applied after merge as a safety net.
 */
function omitSettingsYears(settings, years) {
  if (!years || !years.length) return settings;
  const del = {};
  years.forEach(function(y) { del[String(y)] = true; });
  const out = Object.assign({}, settings);
  ['barriersConfig', 'barriersCustomPassports', 'barriersPK'].forEach(function(key) {
    const src = settings && settings[key];
    if (!src || typeof src !== 'object') return;
    const next = {};
    let changed = false;
    Object.keys(src).forEach(function(y) {
      if (del[String(y)]) { changed = true; return; }
      next[y] = src[y];
    });
...
function mergePasswordsPreserve(currentPw, incomingPw) {
  ...
  out.versions = Object.assign({}, cur.versions || {}, inc.versions || {});
  if (cur.hashes && !out.hashes) out.hashes = cur.hashes;
  ...
  out.versions.orgs = Object.assign({}, (cur.versions && cur.versions.orgs) || {}, (inc.versions && inc.versions.orgs) || {});
  out.hashes = Object.assign({}, cur.hashes || {}, inc.hashes || {});
  if (out.admin && out.admin !== cur.admin) {
    out.versions.admin = (Number((cur.versions || {}).admin) || 1) + 1;
    delete out.hashes.admin;
  }
  if (out.inspector && out.inspector !== cur.inspector) {
    out.versions.inspector = (Number((cur.versions || {}).inspector) || 1) + 1;
    delete out.hashes.inspector;
  }
  if (out.superuser && out.superuser !== cur.superuser) {
    out.versions.superuser = (Number((cur.versions || {}).superuser) || 1) + 1;
    delete out.hashes.superuser;
  }
  Object.keys(out.orgs || {}).forEach(function(name) {
    if (out.orgs[name] && out.orgs[name] !== curOrgs[name]) {
      out.versions.orgs[name] = (Number(((cur.versions || {}).orgs || {})[name]) || 1) + 1;
      out.hashes.orgs = Object.assign({}, out.hashes.orgs || {});
      delete out.hashes.orgs[name];
    }
  });
  return out;
}
...
function validateMasterSettingsWrite(current, incoming) {
  ...
  // No catastrophic-shrink guard for org passwords: deleting organizations
  // through the "Организации" settings screen (checkbox multi-select + "-")
  // is a deliberate admin action and removes the orgs' passwords with them
  // (deleteSelectedOrgs). The shrink guard used to reject such legitimate
  // bulk deletes as 'shrunk_org_passwords', which reverted the whole org
  // deletion on the server. Only guard against wiping the passwords out
  // entirely (handled by empty_org_passwords above).
  ...
  // No catastrophic-shrink guard for orgs count: deleting several
  // organizations at once via the "Организации" settings screen (checkbox
  // multi-select + "-") is a deliberate admin action, not accidental data
  // loss. The shrink guard used to reject such legitimate bulk deletes as
  // 'shrunk_orgs', and the client would then silently revert the deletion
  // (see syncSettingsToServer 409-handling) — the org reappeared after
  // leaving and re-entering the screen. Only guard against orgs being wiped
  // out entirely (handled by empty_orgs above).
...
function saveSettings(data) {
  ...
  // Explicit whole-year deletions (settings → «Барьеры ПК» / «Паспорта КБ»,
  // кнопка «−»). Apply to the baseline BEFORE validation/merge: otherwise
  // mergeSettingsPreserveAll re-adds the removed year from `current` (its
  // stale/partial-payload protection) and the year button + its data
  // "resurrect" after leaving and re-entering the screen.
  const deletedYears = (Array.isArray(payload.deletedBarriersYears) ? payload.deletedBarriersYears : [])
    .map(function(y) { return String(y).trim(); })
    .filter(function(y) { return !!y; });
  delete payload.deletedBarriersYears;
  const base = omitSettingsYears(current, deletedYears);
  const rejectReason = validateMasterSettingsWrite(base, payload);
  if (rejectReason) {
  ...
  const merged = omitSettingsYears(mergeSettingsPreserveAll(base, payload), deletedYears);
