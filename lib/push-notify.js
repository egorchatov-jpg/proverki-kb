...
async function sendViolationPush(record, senderEndpoint) {
  ...
  const checkId = record.checkId || record.id || '';
  ...
  return enqueuePush(function() {
    ...
    const ok = (result && result.sent > 0) ? 'OK' : 'FAIL';
    console.log(`[push] violation ${ok} check=${checkId} org=${record.org||''} barrier=${record.barrier||''} sent=${result.sent}/${result.total} start=${startedAt} done=${doneAt} (push service ack only)`);
    return result;
  });
}
