...
async function sendRecordsChangedPush(senderEndpoint, hint) {
  const payload = {
    type: 'sync',
    silent: true,
    title: 'Проверки КБ',
    body: hint || 'Добавлены или изменены проверки',
    tag: 'records-sync-' + Math.floor(Date.now() / 60000),
  };
  const result = await sendPushPayload(payload, senderEndpoint, { urgency: 'normal' });
