...
(старый адрес `kbcheck.webtm.ru` больше не использовать — часто блокировался сетями)  
...
1. **Timeweb Cloud** → **App Platform** → приложение «Проверки КБ» (`proverkikb.tw1.ru`).
...
6. Проверка: https://proverkikb.tw1.ru/health → `{ "ok": true, "githubPersist": true, "database": "proverki.db" }`.
